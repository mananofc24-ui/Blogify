...
def list_past_blogs() -> List[Path]:
    """
    Returns all generated blog markdown files, newest first.
    """
...
with st.sidebar:
    ...
    value=False,
    help="If enabled, the agent will try to generate AI images using Gemini. If generation fails, the blog will still be created."
    )
...
# --- Images tab ---
    with tab_images:
        ...
        results = out.get("image_results", [])

        if not results:
            st.info("No image generation results.")
        else:
            ...
            c1.metric("Generated", success_count)
            ...
            for i, img in enumerate(results, start=1):
                ...
                if img["status"] == "success":
                    ...
                    st.write("**Filename:**", img["filename"])
                    st.write("**Caption:**", img["caption"])
                    st.write("**Alt text:**", img["alt"])
